                                            ...
                                            <table id="example" class="stripe row-border order-column text-capitalize"
                                                ...
                                                <tbody>
                                                    ...
                                                    @foreach ($users as $user)
                                                        <tr>
                                                            <td> {{ $i }} </td>
                                                            <td> {{ $user->client->raison_sociale?? "----" }} </td>
                                                            <td> {{ $user->name }} </td>
                                                            <td> {{ $user->email }} </td>
                                                            <td>
                                                                @if ($user->status == 'activé')
                                                                    <span class="badge badge-pill badge-success"
                                                                        style="position: relative;"><span class="pulse"
                                                                            style="right:-15px"></span>
                                                                        {{ $user->status }}</span>
                                                                @else
                                                                    <span class="badge badge-pill badge-danger"
                                                                        style="position: relative;"><span
                                                                            class="pulse-danger" style="right:-15px"></span>
                                                                        {{ $user->status }}</span>
                                                                @endif
                                                            </td>
                                                            <td><span class="text-lowercase badge badge-primary">{{ $user->lastLogin() }}</span></td>
                                                            <td>
                                                                @foreach ($user->roles_name as $role)
                                                                    <a href="{{ adminUrl('roles/' . $user->roles[0]->id) }}"
